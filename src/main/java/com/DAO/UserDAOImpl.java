...
//		order by bookId DESC
	...
	public List<AddBook> getRecentBook() {
		List<AddBook> lists = new ArrayList<AddBook>();
		String query = "select * from books where book_categorie=? and book_status=? order by id DESC";
//		order by bookId DESC
		try {
			Connection connection = DBConnect.getConnection();
			PreparedStatement prepareStatement = connection.prepareStatement(query);
...
//		order by bookId DESC
	...
	public boolean payment(Order order) {
		String query = "insert into orders(id,bid,book_name,author_name,price) values(?,?,?,?,?)";
		...
		try {
			...
			prepareStatement.setString(3, order.getBookName());
			prepareStatement.setString(4, order.getAuthorName());
			prepareStatement.setDouble(5, order.getPrice());
	...
	String query = "update books set book_name=?, author=?, price=?, book_status=? where id=?";
	try {
		Connection connection = DBConnect.getConnection();
		PreparedStatement prepareStatement = connection.prepareStatement(query);
	...
	@Override
	public boolean deleteBook(AddBook b) {
	...
	public List<AddBook> getBookBySearch(String ch) {
		...
		String query = "select * from books where book_name like ? or author like ? or book_categorie like ? and book_status=?";
//		order by bookId DESC
		try {
			Connection connection = DBConnect.getConnection();
			PreparedStatement prepareStatement = connection.prepareStatement(query);
